...
let imageMessage = await prepareWAMessageMedia({ image: { url: "https://satganzdevs-api.up.railway.app/api/thmb" } }, { upload: conn.waUploadToServer });
...
nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({buttons:rowr}),contextInfo:{ isForwarded: true, forwardingScore: 1000, forwardedNewsletterMessageInfo: { newsletterJid: global.newsletterJid, serverMessageId: 100, newsletterName: global.newsletterName}, mentionedJid: [...text.matchAll(/@([0-9]{5,16}|0)/g),...title.matchAll(/@([0-9]{5,16}|0)/g),...footer.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + "@s.whatsapp.net") ||''}})}}}), {quoted, userJid: id})
...
break;
case 'call':
buttonParamsJson = JSON.stringify({ display_text: button.text, phone_number: button.id });
break;
case 'url':
buttonParamsJson = JSON.stringify({ display_text: button.text, url: button.id, merchant_url: button.id });
break;
...
return { name: button.type === 'copy' ? 'cta_copy' : (button.type === 'call' ? 'cta_call' : (button.type === 'url' ? 'cta_url' : 'quick_reply')), buttonParamsJson };
});

let imageMessage;
if (options.img) {
const buffer = 
  Buffer.isBuffer(options.img)
    ? options.img
    ...
    : Buffer.alloc(0);
imageMessage = await prepareWAMessageMedia({ image: buffer }, { upload: conn.waUploadToServer });
} else {
imageMessage = await prepareWAMessageMedia({ image: { url: "https://satganzdevs-api.up.railway.app/api/thmb" } }, { upload: conn.waUploadToServer });
}
